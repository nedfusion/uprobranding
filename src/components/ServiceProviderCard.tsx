...
interface ServiceProviderCardProps {
  ...
  onBook?: (serviceProviderId: string) => void;
  onMessage?: (serviceProviderId: string) => void;
  onCall?: (serviceProviderId: string) => void;
}

export function ServiceProviderCard({ serviceProvider, onBook, onMessage, onCall }: ServiceProviderCardProps) {
  return (
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-3 sm:gap-0">
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center text-xs sm:text-sm text-gray-500 mt-1 gap-1 sm:gap-0">
                ...
                <span>{serviceProvider.completedJobs} jobs completed</span>
              </div>
            </div>
          </div>
          ...
          <div className="text-left sm:text-right">
            <p className="text-base sm:text-lg font-semibold text-gray-900">
              ₦{serviceProvider.priceRange.min.toLocaleString()} - ₦{serviceProvider.priceRange.max.toLocaleString()}
            </p>
        ...
        <div className="mb-4">
          <div className="flex flex-wrap gap-1 sm:gap-2">
            {serviceProvider.serviceCategories.map(category => (
              <span
                key={category}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs sm:text-sm text-gray-500 mb-4 gap-2 sm:gap-0">
          ...
          <div className="flex items-center">
            ...
            Responds in {serviceProvider.responseTime}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
          <button 
            onClick={() => onBook?.(serviceProvider.id)}
          ...
          <div className="flex space-x-2 sm:space-x-3">
            <button 
              onClick={() => onMessage?.(serviceProvider.id)}
            ...
            <button 
              onClick={() => onCall?.(serviceProvider.id)}
              className="flex-1 sm:flex-none flex items-center justify-center px-3 sm:px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors text-sm"
            >
